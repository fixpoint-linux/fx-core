// fx-sha384sum.zig — a standalone, Dhall-typed `sha384sum` coreutil.
//
// Computes the SHA-384 (384-bit) message digest of one or more files, or of
// stdin, and prints each digest followed by the file name, byte-identical to
// GNU coreutils sha384sum output.  Pure: no datalog / journal dependency — just
// libc file I/O plus the dhall module for typed arguments and
// std.crypto.hash.sha2.Sha384 for the digest.
//
// Two arg forms, ONE source of truth (schemas/sha384sum.dhall — the fx-ls
// migration template):
//   fx-sha384sum '{ files = [ "/f" ], binary = True }'   Dhall record
//   fx-sha384sum [-b] [FILE...]                           POSIX
//
...
// (src/generated/cli_sha384sum.zig, emitted from schemas/sha384sum.dhall by
...
const cli_sha384sum = @import("cli-sha384sum");
...
const digest_len = Hash.digest_length; // 48

// ---------------------------------------------------------------------------
// CLI option model — GENERATED (single source of truth: schemas/sha384sum.dhall)
// ---------------------------------------------------------------------------

const Options = cli_sha384sum.Options;
...
fn evalDhallArgs(src: [:0]const u8, gpa: Allocator) !Options {
    ...
    if (t == null) {
        std.debug.print("fx-sha384sum: dhall parse error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
        return error.DhallParse;
    }
    const ty = typecheck.infer_type(&p, t.?, &err);
    if (ty == null) {
        std.debug.print("fx-sha384sum: dhall type error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (normalize.normalize_has_error()) {
        ...
        std.debug.print("fx-sha384sum: dhall normalize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (!serialize.term_to_json(out, nf, &err)) {
        std.debug.print("fx-sha384sum: dhall serialize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    const opts = jsonParseOpts(ob.items, buf) orelse {
        std.debug.print("fx-sha384sum: could not parse dhall record fields from JSON: {s}\n", .{ob.items});
...
/// Locate schemas/sha384sum.dhall (tests run from varying CWDs).  Caller frees.
fn sha384SchemaSrc() [:0]u8 {
    return cli.readSchemaFile(std.testing.allocator, &.{ "schemas/sha384sum.dhall", "fx-core/schemas/sha384sum.dhall" }) catch
        @panic("cannot locate schemas/sha384sum.dhall (run tests from the fx-core root)");
}

/// One differential vector for fx-sha384sum — a one-line wrapper over the
...
fn expectPosixEqualsRecord(argv: []const []const u8, user_record: [:0]const u8) !void {
    return cli.expectPosixEqualsRecord(cli_sha384sum, &.{ "schemas/sha384sum.dhall", "fx-core/schemas/sha384sum.dhall" }, evalDhallArgs, argv, user_record);
}

test "DIFFERENTIAL: generated parsePosix equals the Dhall-record form (matrix)" {
    // empty argv == the all-defaults record (files = [], binary = False —
    // a legal digest-stdin run)
    try expectPosixEqualsRecord(&.{"fx-sha384sum"}, "{ }");
    // -b alone, the --binary long alias, and the -b short cluster
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "-b" }, "{ binary = True }");
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "--binary" }, "{ binary = True }");
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "-bb" }, "{ binary = True }");
    // operands in argv order, with and without the flag
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "a" }, "{ files = [ \"a\" ] }");
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "-b", "a", "b" }, "{ files = [ \"a\", \"b\" ], binary = True }");
    // a bare '-' is an operand, not a flag
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "-" }, "{ files = [ \"-\" ] }");
    // `--` ends flag parsing: -b after it is a FILE operand
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "--", "-b" }, "{ files = [ \"-b\" ] }");
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "-b", "--", "a" }, "{ files = [ \"a\" ], binary = True }");
    // flag-operand interleave (GNU parity)
    try expectPosixEqualsRecord(&.{ "fx-sha384sum", "a", "-b", "b" }, "{ files = [ \"a\", \"b\" ], binary = True }");
}
...
test "DIFFERENTIAL: rejection parity — both arg forms fail loudly" {
    ...
    try std.testing.expectError(error.UnknownOption, cli_sha384sum.parsePosix(&.{ "fx-sha384sum", "-c" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_sha384sum.parsePosix(&.{ "fx-sha384sum", "--check" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_sha384sum.parsePosix(&.{ "fx-sha384sum", "--binary=1" }, gpa));
    // a cluster with an unknown letter is an unknown option, never an operand
    try std.testing.expectError(error.UnknownOption, cli_sha384sum.parsePosix(&.{ "fx-sha384sum", "-bz" }, gpa));
    ...
    // above.
    const schema_src = sha384SchemaSrc();
...
test "sha384 known-answer: hi-newline" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("hi\n", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("4284b5694ca6c0d2cf4789a0b95ac8025c818de52304364be7cd2981b2d2edc685b322277ec25819962413d8c9b2c1f5", &hexbuf);
}

test "sha384 known-answer: empty" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("38b060a751ac96384cd9327eb1b1e36a21fdb71114be07434c0cc7bf63f6e1da274edebfe76f65fbd51ad2f14898b95b", &hexbuf);
}

test "sha384 file round-trip" {
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len >= 2 and args[1].len > 0 and args[1][0] == '{') {
        ...
        // the GENERATED parser (schemas/sha384sum.dhall ->
        // src/generated/cli_sha384sum.zig); equality with the record form
        // above is pinned by the differential tests (expectPosixEqualsRecord)
        opts = try cli_sha384sum.parsePosix(args, opt_alloc);
    }
...
fn digestFile(path: []const u8) ![digest_len]u8 {
    ...
    if (fd < 0) {
        std.debug.print("fx-sha384sum: cannot open '{s}'\n", .{path});
