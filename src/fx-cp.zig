...
//
// Two arg forms, ONE source of truth (schemas/cp.dhall):
//   fx-cp '{ src = "/a", dst = "/b" }'                Dhall record
//   fx-cp SRC DST                                     POSIX fallback
//
// The POSIX form is parsed by the GENERATED parser (src/generated/cli_cp.zig,
// emitted from schemas/cp.dhall by src/tools/fx-clijson.zig — pure Zig, no
// dhall at runtime; `zig build gen-cli-check` gates the regen).  cp has NO
// flags in v1 (no -r: a directory src is the 'omitting directory' error), so
// every "-..." token is error.UnknownOption.  src/dst are Text with ""
// placeholder defaults (a single positional must bind a plain Text field):
// the generated parser fills the two slots in order and leaves the
// exactly-two count to the runtime check in main() below (the ln/chown
// precedent) — one operand now PARSES (dst stays "") where the hand parser
// errored BadArgs on any count != 2.  Equality of the two arg forms is pinned
// field-for-field by the differential test below (the fx-ls/fx-ln template).
//
...
// ---------------------------------------------------------------------------
// CLI option model — GENERATED (single source of truth: schemas/cp.dhall)
// ---------------------------------------------------------------------------

const Options = cli_cp.Options; // src/dst : Text, "" placeholder defaults
const parsePosixArgs = cli_cp.parsePosix; // the generated POSIX parser
...
/// One differential vector for fx-cp — a one-line wrapper over the SHARED
/// generic runner (fx-cli.expectPosixEqualsRecord).
fn expectPosixEqualsRecord(argv: []const []const u8, user_record: [:0]const u8) !void {
    return cli.expectPosixEqualsRecord(cli_cp, &.{ "schemas/cp.dhall", "fx-core/schemas/cp.dhall" }, evalDhallOpts, argv, user_record);
}

test "DIFFERENTIAL: generated parsePosix equals the Dhall-record form (matrix)" {
    // --- the two positionals, in order and with a later flag-shaped
    // operand (cp has no flags, so a "-..." token AFTER `--` is a plain
    // operand) ---
    try expectPosixEqualsRecord(&.{ "fx-cp", "/a", "/b" }, "{ src = \"/a\", dst = \"/b\" }");
    try expectPosixEqualsRecord(&.{ "fx-cp", "--", "-x", "/b" }, "{ src = \"-x\", dst = \"/b\" }");

    // --- empty argv: both stay at the schema's "" placeholder defaults
    // (the exactly-two count is runtime, in main) ---
    try expectPosixEqualsRecord(&.{"fx-cp"}, "{ }");
    ...
    try expectPosixEqualsRecord(&.{ "fx-cp", "a b.txt", "say \"hi\".txt" }, "{ src = \"a b.txt\", dst = \"say \\\"hi\\\".txt\" }");
}
...
test "DIFFERENTIAL: rejection parity — both arg forms fail loudly" {
    ...
    // cp has NO flags (schemas/cp.dhall flags = []): any option-ish token
    // is UnknownOption — including the -r spelling GNU would recurse with
    // (the deliberate v1 scope cut; a directory src stays OmittingDir)
    try std.testing.expectError(error.UnknownOption, cli_cp.parsePosix(&.{ "fx-cp", "-r", "/a", "/b" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_cp.parsePosix(&.{ "fx-cp", "-Zz", "/a", "/b" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_cp.parsePosix(&.{ "fx-cp", "--bogus" }, gpa));
    ...
    try std.testing.expectError(error.UnexpectedOperand, cli_cp.parsePosix(&.{ "fx-cp", "/a", "/b", "/c" }, gpa));
    ...
    const schema_src = cli.readSchemaFile(std.testing.allocator, &.{ "schemas/cp.dhall", "fx-core/schemas/cp.dhall" }) catch
        @panic("cannot locate schemas/cp.dhall (run tests from the fx-core root)");
    ...
    // MissingValue / BadValue: cp has NO Value flag and no numeric field,
...
pub fn main(init: std.process.Init) !void {
    ...
    if (opts.src.len == 0 or opts.dst.len == 0) {
        std.debug.print("fx-cp: exactly SRC and DST required\n", .{});
        return error.BadArgs;
    }
    const src = opts.src;
    const dst = opts.dst;
