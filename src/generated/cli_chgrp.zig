// cli_chgrp.zig — GENERATED by src/tools/fx-clijson.zig from schemas/chgrp.dhall.
...
fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator, paths_items: *std.ArrayList([]const u8)) ParseError!void {
    if (next_pos.* == 0) {
        o.group = gpa.dupe(u8, arg) catch return error.OutOfMemory;
        next_pos.* = 1;
        return;
    }
...
pub fn usage() []const u8 {
    return "usage: fx-chgrp [GROUP] [FILE...]\n";
}

test "cli_chgrp: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-chgrp" };
...
test "cli_chgrp: operand binds group" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-chgrp", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("op0", o.group);
}

test "cli_chgrp: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-chgrp", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_chgrp: usage non-empty" {
