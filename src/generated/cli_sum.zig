// cli_sum.zig — GENERATED by src/tools/fx-clijson.zig from schemas/sum.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (!matched and (std.mem.eql(u8, a, "-s") or std.mem.eql(u8, a, "--sysv"))) {
            o.sysv = true;
            matched = true;
        }
        if (!matched) {
...
pub fn usage() []const u8 {
    return "usage: fx-sum [OPTIONS] [FILE...]\n";
}

test "cli_sum: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sum" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 0), o.files.len);
    try std.testing.expectEqual(false, o.sysv);
}

test "cli_sum: -s binds sysv" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sum", "-s" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expect(o.sysv);
}

test "cli_sum: cluster with unknown letter rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sum", "-sA" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_sum: operand binds files" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sum", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 1), o.files.len);
    try std.testing.expectEqualStrings("op0", o.files[0]);
}

test "cli_sum: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sum", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_sum: usage non-empty" {
