// cli_what.zig — GENERATED by src/tools/fx-clijson.zig from schemas/what.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (!matched) {
            if (a.len > 1 and a[0] == '-') {
                std.debug.print("fx-what: unknown option '{s}'\n", .{ a });
...
fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator) ParseError!void {
    ...
    std.debug.print("fx-what: unexpected operand '{s}'\n", .{ arg });
    return error.UnexpectedOperand;
}

pub fn usage() []const u8 {
    return "usage: fx-what [OPTIONS] [PATH]\n";
}

test "cli_what: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-what" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("", o.operand);
    try std.testing.expect(o.store == null);
}

test "cli_what: --as-of binds as_of" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
...
test "cli_what: --store binds store" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
...
test "cli_what: --as-of=value binds as_of" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-what", "--as-of=7" };
...
test "cli_what: operand binds operand" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-what", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("op0", o.operand);
}

test "cli_what: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-what", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_what: usage non-empty" {
