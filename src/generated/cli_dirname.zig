// cli_dirname.zig — GENERATED by src/tools/fx-clijson.zig from schemas/dirname.dhall.
...
pub fn usage() []const u8 {
    return "usage: fx-dirname [NAME...]\n";
}

test "cli_dirname: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-dirname" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 0), o.names.len);
}

test "cli_dirname: operand binds names" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-dirname", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 1), o.names.len);
    try std.testing.expectEqualStrings("op0", o.names[0]);
}

test "cli_dirname: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-dirname", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_dirname: usage non-empty" {
