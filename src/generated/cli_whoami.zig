// cli_whoami.zig — GENERATED by src/tools/fx-clijson.zig from schemas/whoami.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    ...
    while (i < args.len) : (i += 1) {
        const a = args[i];
        if (after_ddash) {
            std.debug.print("fx-whoami: unexpected operand '{s}'\n", .{ a });
        ...
        if (a.len > 1 and a[0] == '-') {
            std.debug.print("fx-whoami: unknown option '{s}'\n", .{ a });
...
pub fn usage() []const u8 {
    return "usage: fx-whoami\n";
}

test "cli_whoami: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-whoami" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(true, o.nothing);
}

test "cli_whoami: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-whoami", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_whoami: usage non-empty" {
