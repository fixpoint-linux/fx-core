// cli_head.zig — GENERATED by src/tools/fx-clijson.zig from schemas/head.dhall.
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (!matched and std.mem.startsWith(u8, a, "--lines=")) {
            o.n = std.fmt.parseInt(u64, args[i][8..], 10) catch {
                std.debug.print("fx-head: option '--lines': '{s}' is not a Natural (u64)\n", .{ args[i][8..] });
                return error.BadValue;
            };
            matched = true;
        }
...
fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator) ParseError!void {
    ...
    std.debug.print("fx-head: unexpected operand '{s}'\n", .{ arg });
    return error.UnexpectedOperand;
}

pub fn usage() []const u8 {
    return "usage: fx-head [OPTIONS] [FILE]\n";
}

test "cli_head: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-head" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("", o.input);
    try std.testing.expectEqual(@as(u64, 10), o.n);
}

test "cli_head: -n binds n" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-head", "-n", "7" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(u64, 7), o.n);
}

test "cli_head: --lines=value binds n" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
...
test "cli_head: operand binds input" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-head", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("op0", o.input);
}

test "cli_head: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-head", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_head: usage non-empty" {
