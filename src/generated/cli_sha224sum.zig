// cli_sha224sum.zig — GENERATED by src/tools/fx-clijson.zig from schemas/sha224sum.dhall.
...
pub fn usage() []const u8 {
    return "usage: fx-sha224sum [OPTIONS] [FILE...]\n";
}

test "cli_sha224sum: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sha224sum" };
...
test "cli_sha224sum: -b binds binary" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sha224sum", "-b" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expect(o.binary);
}

test "cli_sha224sum: cluster with unknown letter rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sha224sum", "-bA" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_sha224sum: operand binds files" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sha224sum", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 1), o.files.len);
    try std.testing.expectEqualStrings("op0", o.files[0]);
}

test "cli_sha224sum: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-sha224sum", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_sha224sum: usage non-empty" {
