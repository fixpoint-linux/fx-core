// cli_chown.zig — GENERATED by src/tools/fx-clijson.zig from schemas/chown.dhall.
...
fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator, paths_items: *std.ArrayList([]const u8)) ParseError!void {
    if (next_pos.* == 0) {
        o.owner = gpa.dupe(u8, arg) catch return error.OutOfMemory;
        next_pos.* = 1;
        return;
    }
...
pub fn usage() []const u8 {
    return "usage: fx-chown [OWNER] [FILE...]\n";
}

test "cli_chown: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-chown" };
...
test "cli_chown: operand binds owner" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-chown", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("op0", o.owner);
}

test "cli_chown: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-chown", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_chown: usage non-empty" {
