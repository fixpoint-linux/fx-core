// cli_realpath.zig — GENERATED by src/tools/fx-clijson.zig from schemas/realpath.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    var names_items = std.ArrayList([]const u8).empty;
    errdefer names_items.deinit(gpa);
    ...
    while (i < args.len) : (i += 1) {
        const a = args[i];
        if (after_ddash) {
            try bindOperand(a, &o, &next_pos, gpa, &names_items);
            continue;
        }
        if (std.mem.eql(u8, a, "--")) {
            after_ddash = true;
    ...
    if (names_items.items.len > 0) o.names = names_items.toOwnedSlice(gpa) catch return error.OutOfMemory;
    return o;
}

fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator, names_items: *std.ArrayList([]const u8)) ParseError!void {
    _ = o;
    _ = next_pos;
    names_items.append(gpa, gpa.dupe(u8, arg) catch return error.OutOfMemory) catch return error.OutOfMemory;
}

pub fn usage() []const u8 {
    return "usage: fx-realpath [FILE...]\n";
}

test "cli_realpath: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-realpath" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 0), o.names.len);
}

test "cli_realpath: operand binds names" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-realpath", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 1), o.names.len);
    try std.testing.expectEqualStrings("op0", o.names[0]);
}

test "cli_realpath: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-realpath", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_realpath: usage non-empty" {
