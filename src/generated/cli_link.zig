// cli_link.zig — GENERATED by src/tools/fx-clijson.zig from schemas/link.dhall.
...
fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator) ParseError!void {
    if (next_pos.* == 0) {
        o.old = gpa.dupe(u8, arg) catch return error.OutOfMemory;
        next_pos.* = 1;
        return;
    }
    if (next_pos.* == 1) {
        o.new = gpa.dupe(u8, arg) catch return error.OutOfMemory;
        next_pos.* = 2;
        return;
    }
    std.debug.print("fx-link: unexpected operand '{s}'\n", .{ arg });
    return error.UnexpectedOperand;
}

pub fn usage() []const u8 {
    return "usage: fx-link [OLD] [NEW]\n";
}

test "cli_link: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-link" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("", o.new);
    try std.testing.expectEqualStrings("", o.old);
}

test "cli_link: operand binds old" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-link", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("op0", o.old);
}

test "cli_link: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-link", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_link: usage non-empty" {
