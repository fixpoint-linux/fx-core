// cli_df.zig — GENERATED by src/tools/fx-clijson.zig from schemas/df.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (!matched) {
            if (a.len > 1 and a[0] == '-') {
                std.debug.print("fx-df: unknown option '{s}'\n", .{ a });
...
fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator) ParseError!void {
    ...
    std.debug.print("fx-df: unexpected operand '{s}'\n", .{ arg });
    return error.UnexpectedOperand;
}

pub fn usage() []const u8 {
    return "usage: fx-df [OPTIONS] [PATH]\n";
}

test "cli_df: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-df" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("", o.path);
    try std.testing.expectEqual(false, o.rows);
}

test "cli_df: --rows binds rows" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-df", "--rows" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expect(o.rows);
}

test "cli_df: operand binds path" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-df", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("op0", o.path);
}

test "cli_df: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-df", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_df: usage non-empty" {
