// cli_tee.zig — GENERATED by src/tools/fx-clijson.zig from schemas/tee.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (!matched and (std.mem.eql(u8, a, "-a") or std.mem.eql(u8, a, "--append"))) {
            o.append = true;
            matched = true;
        }
        if (!matched) {
            if (a.len > 1 and a[0] == '-') {
                std.debug.print("fx-tee: unknown option '{s}'\n", .{ a });
...
pub fn usage() []const u8 {
    return "usage: fx-tee [OPTIONS] [FILE...]\n";
}

test "cli_tee: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-tee" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(false, o.append);
    try std.testing.expectEqual(@as(usize, 0), o.files.len);
}

test "cli_tee: -a binds append" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-tee", "-a" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expect(o.append);
}

test "cli_tee: cluster with unknown letter rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-tee", "-aA" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_tee: operand binds files" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-tee", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 1), o.files.len);
    try std.testing.expectEqualStrings("op0", o.files[0]);
}

test "cli_tee: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-tee", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_tee: usage non-empty" {
