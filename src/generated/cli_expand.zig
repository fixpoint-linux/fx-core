// cli_expand.zig — GENERATED by src/tools/fx-clijson.zig from schemas/expand.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (!matched and std.mem.startsWith(u8, a, "--tabs=")) {
            o.tabstop = std.fmt.parseInt(u64, args[i][7..], 10) catch {
                std.debug.print("fx-expand: option '--tabs': '{s}' is not a Natural (u64)\n", .{ args[i][7..] });
                return error.BadValue;
            };
            matched = true;
        }
...
pub fn usage() []const u8 {
    return "usage: fx-expand [OPTIONS] [FILE...]\n";
}

test "cli_expand: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-expand" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 0), o.files.len);
    try std.testing.expectEqual(@as(u64, 8), o.tabstop);
}

test "cli_expand: -t binds tabstop" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-expand", "-t", "7" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(u64, 7), o.tabstop);
}

test "cli_expand: --tabs=value binds tabstop" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
...
test "cli_expand: operand binds files" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-expand", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqual(@as(usize, 1), o.files.len);
    try std.testing.expectEqualStrings("op0", o.files[0]);
}

test "cli_expand: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-expand", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_expand: usage non-empty" {
