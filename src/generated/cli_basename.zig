// cli_basename.zig — GENERATED by src/tools/fx-clijson.zig from schemas/basename.dhall.
...
pub fn parsePosix(args: []const []const u8, gpa: Allocator) ParseError!Options {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (!matched and (std.mem.eql(u8, a, "-a") or std.mem.eql(u8, a, "--multiple"))) {
            o.all = true;
            matched = true;
        }
        if (!matched) {
...
fn bindOperand(arg: []const u8, o: *Options, next_pos: *usize, gpa: Allocator) ParseError!void {
    ...
    if (next_pos.* == 1) {
        o.suffix = gpa.dupe(u8, arg) catch return error.OutOfMemory;
        next_pos.* = 2;
        return;
    }
    std.debug.print("fx-basename: unexpected operand '{s}'\n", .{ arg });
    return error.UnexpectedOperand;
}

pub fn usage() []const u8 {
    return "usage: fx-basename [OPTIONS] [NAME] [SUFFIX]\n";
}

test "cli_basename: empty argv yields the dflt defaults" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-basename" };
    ...
    try std.testing.expectEqualStrings("", o.suffix);
}

test "cli_basename: -a binds all" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-basename", "-a" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expect(o.all);
}

test "cli_basename: cluster with unknown letter rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-basename", "-aA" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_basename: operand binds input" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-basename", "op0" };
    const o = try parsePosix(&argv, gpa);
    try std.testing.expectEqualStrings("op0", o.input);
}

test "cli_basename: unknown option rejected" {
    var arena_state = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_state.deinit();
    const gpa = arena_state.allocator();
    const argv = [_][]const u8{ "fx-basename", "-Zz" };
    try std.testing.expectError(error.UnknownOption, parsePosix(&argv, gpa));
}

test "cli_basename: usage non-empty" {
