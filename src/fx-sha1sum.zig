// fx-sha1sum.zig — a standalone, Dhall-typed `sha1sum` coreutil.
//
// Computes the SHA-1 (160-bit) message digest of one or more files, or of
// stdin, and prints each digest followed by the file name, byte-identical to
// GNU coreutils sha1sum output.  Pure: no datalog / journal dependency — just
...
fn evalDhallArgs(src: [:0]const u8, gpa: Allocator) !Options {
    ...
    if (ty == null) {
        std.debug.print("fx-sha1sum: dhall type error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (normalize.normalize_has_error()) {
        ...
        std.debug.print("fx-sha1sum: dhall normalize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (!serialize.term_to_json(out, nf, &err)) {
        std.debug.print("fx-sha1sum: dhall serialize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    const opts = jsonParseOpts(ob.items, buf) orelse {
        std.debug.print("fx-sha1sum: could not parse dhall record fields from JSON: {s}\n", .{ob.items});
...
test "sha1 known-answer: hi-newline" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("hi\n", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("55ca6286e3e4f4fba5d0448333fa99fc5a404a73", &hexbuf);
}

test "sha1 known-answer: empty" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("da39a3ee5e6b4b0d3255bfef95601890afd80709", &hexbuf);
}

test "sha1 file round-trip" {
...
fn digestFile(path: []const u8) ![digest_len]u8 {
    ...
    if (fd < 0) {
        std.debug.print("fx-sha1sum: cannot open '{s}'\n", .{path});
