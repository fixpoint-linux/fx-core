// fx-sha512sum.zig — a standalone, Dhall-typed `sha512sum` coreutil.
//
// Computes the SHA-512 (512-bit) message digest of one or more files, or of
// stdin, and prints each digest followed by the file name, byte-identical to
// GNU coreutils sha512sum output.  Pure: no datalog / journal dependency — just
// libc file I/O plus the dhall module for typed arguments and
// std.crypto.hash.sha2.Sha512 for the digest.
//
// Two arg forms, ONE source of truth (schemas/sha512sum.dhall — the fx-ls
// migration template):
//   fx-sha512sum '{ files = [ "/f" ], binary = True }'   Dhall record
//   fx-sha512sum [-b] [FILE...]                           POSIX
//
...
// (src/generated/cli_sha512sum.zig, emitted from schemas/sha512sum.dhall by
...
const cli_sha512sum = @import("cli-sha512sum");
...
const digest_len = Hash.digest_length; // 64

// ---------------------------------------------------------------------------
// CLI option model — GENERATED (single source of truth: schemas/sha512sum.dhall)
// ---------------------------------------------------------------------------

const Options = cli_sha512sum.Options;
...
fn evalDhallArgs(src: [:0]const u8, gpa: Allocator) !Options {
    ...
    if (t == null) {
        std.debug.print("fx-sha512sum: dhall parse error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
        return error.DhallParse;
    }
    const ty = typecheck.infer_type(&p, t.?, &err);
    if (ty == null) {
        std.debug.print("fx-sha512sum: dhall type error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (normalize.normalize_has_error()) {
        ...
        std.debug.print("fx-sha512sum: dhall normalize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (!serialize.term_to_json(out, nf, &err)) {
        std.debug.print("fx-sha512sum: dhall serialize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    const opts = jsonParseOpts(ob.items, buf) orelse {
        std.debug.print("fx-sha512sum: could not parse dhall record fields from JSON: {s}\n", .{ob.items});
...
/// Locate schemas/sha512sum.dhall (tests run from varying CWDs).  Caller frees.
fn shaSchemaSrc() [:0]u8 {
    return cli.readSchemaFile(std.testing.allocator, &.{ "schemas/sha512sum.dhall", "fx-core/schemas/sha512sum.dhall" }) catch
        @panic("cannot locate schemas/sha512sum.dhall (run tests from the fx-core root)");
}

/// One differential vector for fx-sha512sum — a one-line wrapper over the
...
fn expectPosixEqualsRecord(argv: []const []const u8, user_record: [:0]const u8) !void {
    return cli.expectPosixEqualsRecord(cli_sha512sum, &.{ "schemas/sha512sum.dhall", "fx-core/schemas/sha512sum.dhall" }, evalDhallArgs, argv, user_record);
}

test "DIFFERENTIAL: generated parsePosix equals the Dhall-record form (matrix)" {
    // empty argv == the all-defaults record (files = [], binary = False —
    // a legal digest-stdin run)
    try expectPosixEqualsRecord(&.{"fx-sha512sum"}, "{ }");
    // -b alone, the --binary long alias, and the -b short cluster
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "-b" }, "{ binary = True }");
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "--binary" }, "{ binary = True }");
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "-bb" }, "{ binary = True }");
    // operands in argv order, with and without the flag
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "a" }, "{ files = [ \"a\" ] }");
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "-b", "a", "b" }, "{ files = [ \"a\", \"b\" ], binary = True }");
    // a bare '-' is an operand, not a flag
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "-" }, "{ files = [ \"-\" ] }");
    // `--` ends flag parsing: -b after it is a FILE operand
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "--", "-b" }, "{ files = [ \"-b\" ] }");
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "-b", "--", "a" }, "{ files = [ \"a\" ], binary = True }");
    // flag-operand interleave (GNU parity)
    try expectPosixEqualsRecord(&.{ "fx-sha512sum", "a", "-b", "b" }, "{ files = [ \"a\", \"b\" ], binary = True }");
}
...
test "DIFFERENTIAL: rejection parity — both arg forms fail loudly" {
    ...
    try std.testing.expectError(error.UnknownOption, cli_sha512sum.parsePosix(&.{ "fx-sha512sum", "-c" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_sha512sum.parsePosix(&.{ "fx-sha512sum", "--check" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_sha512sum.parsePosix(&.{ "fx-sha512sum", "--binary=1" }, gpa));
    // a cluster with an unknown letter is an unknown option, never an operand
    try std.testing.expectError(error.UnknownOption, cli_sha512sum.parsePosix(&.{ "fx-sha512sum", "-bz" }, gpa));
...
test "sha512 known-answer: hi-newline" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("hi\n", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("d78abb0542736865f94704521609c230dac03a2f369d043ac212d6933b91410e06399e37f9c5cc88436a31737330c1c8eccb2c2f9f374d62f716432a32d50fac", &hexbuf);
}

test "sha512 known-answer: empty" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e", &hexbuf);
}

test "sha512 file round-trip" {
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len >= 2 and args[1].len > 0 and args[1][0] == '{') {
        ...
        // the GENERATED parser (schemas/sha512sum.dhall ->
        // src/generated/cli_sha512sum.zig); equality with the record form
        // above is pinned by the differential tests (expectPosixEqualsRecord)
        opts = try cli_sha512sum.parsePosix(args, opt_alloc);
    }
...
fn digestFile(path: []const u8) ![digest_len]u8 {
    ...
    if (fd < 0) {
        std.debug.print("fx-sha512sum: cannot open '{s}'\n", .{path});
