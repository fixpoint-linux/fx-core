// fx-sha224sum.zig — a standalone, Dhall-typed `sha224sum` coreutil.
//
// Computes the SHA-224 (224-bit) message digest of one or more files, or of
// stdin, and prints each digest followed by the file name, byte-identical to
// GNU coreutils sha224sum output.  Pure: no datalog / journal dependency — just
// libc file I/O plus the dhall module for typed arguments and
// std.crypto.hash.sha2.Sha224 for the digest.
//
// Two arg forms, ONE source of truth (schemas/sha224sum.dhall — the fx-ls
// migration template):
//   fx-sha224sum '{ files = [ "/f" ], binary = True }'   Dhall record
//   fx-sha224sum [-b] [FILE...]                           POSIX
//
...
// (src/generated/cli_sha224sum.zig, emitted from schemas/sha224sum.dhall by
...
const cli_sha224sum = @import("cli-sha224sum");
...
const Hash = std.crypto.hash.sha2.Sha224;
const digest_len = Hash.digest_length; // 28

// ---------------------------------------------------------------------------
// CLI option model — GENERATED (single source of truth: schemas/sha224sum.dhall)
// ---------------------------------------------------------------------------

const Options = cli_sha224sum.Options;
...
fn evalDhallArgs(src: [:0]const u8, gpa: Allocator) !Options {
    ...
    if (t == null) {
        std.debug.print("fx-sha224sum: dhall parse error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
        return error.DhallParse;
    }
    const ty = typecheck.infer_type(&p, t.?, &err);
    if (ty == null) {
        std.debug.print("fx-sha224sum: dhall type error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (normalize.normalize_has_error()) {
        ...
        std.debug.print("fx-sha224sum: dhall normalize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    if (!serialize.term_to_json(out, nf, &err)) {
        std.debug.print("fx-sha224sum: dhall serialize error: {s}\n", .{std.mem.sliceTo(&err.msg, 0)});
    ...
    const opts = jsonParseOpts(ob.items, buf) orelse {
        std.debug.print("fx-sha224sum: could not parse dhall record fields from JSON: {s}\n", .{ob.items});
...
/// Locate schemas/sha224sum.dhall (tests run from varying CWDs).  Caller frees.
fn sha224SchemaSrc() [:0]u8 {
    return cli.readSchemaFile(std.testing.allocator, &.{ "schemas/sha224sum.dhall", "fx-core/schemas/sha224sum.dhall" }) catch
        @panic("cannot locate schemas/sha224sum.dhall (run tests from the fx-core root)");
}

/// One differential vector for fx-sha224sum — a one-line wrapper over the
...
fn expectPosixEqualsRecord(argv: []const []const u8, user_record: [:0]const u8) !void {
    return cli.expectPosixEqualsRecord(cli_sha224sum, &.{ "schemas/sha224sum.dhall", "fx-core/schemas/sha224sum.dhall" }, evalDhallArgs, argv, user_record);
}

test "DIFFERENTIAL: generated parsePosix equals the Dhall-record form (matrix)" {
    // empty argv == the all-defaults record (files = [], binary = False —
    // a legal digest-stdin run)
    try expectPosixEqualsRecord(&.{"fx-sha224sum"}, "{ }");
    // -b alone, the --binary long alias, and the -b short cluster
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "-b" }, "{ binary = True }");
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "--binary" }, "{ binary = True }");
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "-bb" }, "{ binary = True }");
    // operands in argv order, with and without the flag
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "a" }, "{ files = [ \"a\" ] }");
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "-b", "a", "b" }, "{ files = [ \"a\", \"b\" ], binary = True }");
    // a bare '-' is an operand, not a flag
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "-" }, "{ files = [ \"-\" ] }");
    // `--` ends flag parsing: -b after it is a FILE operand
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "--", "-b" }, "{ files = [ \"-b\" ] }");
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "-b", "--", "a" }, "{ files = [ \"a\" ], binary = True }");
    // flag-operand interleave (GNU parity)
    try expectPosixEqualsRecord(&.{ "fx-sha224sum", "a", "-b", "b" }, "{ files = [ \"a\", \"b\" ], binary = True }");
}
...
test "DIFFERENTIAL: rejection parity — both arg forms fail loudly" {
    ...
    try std.testing.expectError(error.UnknownOption, cli_sha224sum.parsePosix(&.{ "fx-sha224sum", "-c" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_sha224sum.parsePosix(&.{ "fx-sha224sum", "--check" }, gpa));
    try std.testing.expectError(error.UnknownOption, cli_sha224sum.parsePosix(&.{ "fx-sha224sum", "--binary=1" }, gpa));
    // a cluster with an unknown letter is an unknown option, never an operand
    try std.testing.expectError(error.UnknownOption, cli_sha224sum.parsePosix(&.{ "fx-sha224sum", "-bz" }, gpa));
    ...
    // above.
    const schema_src = sha224SchemaSrc();
...
test "sha224 known-answer: hi-newline" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("hi\n", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("0689e590864cee67b037c8f4c390a75a192bb01160cc9c21374afb12", &hexbuf);
}

test "sha224 known-answer: empty" {
    var out: [digest_len]u8 = undefined;
    Hash.hash("", &out, .{});
    var hexbuf: [digest_len * 2]u8 = undefined;
    hexDigest(out, &hexbuf);
    try std.testing.expectEqualStrings("d14a028c2a3a2bc9476102bb288234c415a2b01f828ea62ac5b3e42f", &hexbuf);
}

test "sha224 file round-trip" {
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len >= 2 and args[1].len > 0 and args[1][0] == '{') {
        ...
        // the GENERATED parser (schemas/sha224sum.dhall ->
        // src/generated/cli_sha224sum.zig); equality with the record form
        // above is pinned by the differential tests (expectPosixEqualsRecord)
        opts = try cli_sha224sum.parsePosix(args, opt_alloc);
    }
...
fn digestFile(path: []const u8) ![digest_len]u8 {
    ...
    if (fd < 0) {
        std.debug.print("fx-sha224sum: cannot open '{s}'\n", .{path});
