-- schemas/sha384sum.dhall — the single source of truth for the
-- fx-sha384sum command interface.
--
--   ty    mirrors fx-sha384sum.zig's Options struct
--         (fx-sha384sum.zig:62-66):
...
--         (fx-sha384sum.zig:213-218 — the one-file Dhall limitation);
...
--         `{ input = None Text }` (fx-sha384sum.zig:274-279); against
...
--         name '-' (fx-sha384sum.zig:15-16, main branch
--         fx-sha384sum.zig:382-396).
--   posix fx-sha384sum.zig:225-242 parsePosixArgs:
...
in
{ doc = Some "print or check SHA-384 checksums"
, ty =
    { binary : Bool
    , files : List Text
    }
